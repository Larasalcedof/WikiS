document.addEventListener("DOMContentLoaded", function () {
  ...
  var descripcionTextarea = document.getElementById("descripcion"); // Corregir el nombre del ID
  ...
  function attachInputEvent(inputElement, maxChars) {
    inputElement.addEventListener("input", function () {
      updateCharCount(inputElement, maxChars);
    });
  }

  attachInputEvent(nombresInput, 100);
  attachInputEvent(apellidosInput, 100);
  attachInputEvent(correoInput, 100);
  attachInputEvent(semestreInput, 2);
  attachInputEvent(descripcionTextarea, 500);
  ...
  document.getElementById("submission").addEventListener("click", function (event) {
    ...
    alert(
      "¡Formulario válido! La información ha sido enviada correctamente."
    );
